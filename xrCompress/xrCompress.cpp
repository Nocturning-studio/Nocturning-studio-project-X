...
BOOL testSKIP(LPCSTR path)
{
    ...
    if (0 == stricmp(p_ext, ".swatch"))
        return TRUE;
    if (0 == stricmp(p_ext, ".db"))
        return TRUE;
    if (PatternMatch(p_ext, "*avi*"))
        return TRUE;
    if (0 == stricmp(p_ext, ".key"))
        return TRUE;
    ...
    if (0 == stricmp(p_ext, ".smf"))
        return TRUE;
    if (0 == stricmp(p_ext, ".uvm"))
        return TRUE;
    if (0 == stricmp(p_ext, ".raw"))
        return TRUE;
...
IC void write_file_header(LPCSTR file_name, const u32 &crc, const u32 &ptr, const u32 &size_real,
                          const u32 &size_compressed)
{
        ...
        else
        {
            ...
            if (0 != c_size_real)
            {
                ...
                t_compress.Begin();
                {
                    ...
                    if (bFast)
                    {
                        R_ASSERT(LZO_E_OK == lzo1x_1_compress((u8 *)src->pointer(), c_size_real, c_data,
                                                              &c_size_compressed, c_heap));
                    }
                    else
                    {
                        R_ASSERT(LZO_E_OK == lzo1x_999_compress((u8 *)src->pointer(), c_size_real, c_data,
                                                                &c_size_compressed, c_heap));
                    }
                }
                t_compress.End();
...
void CompressList(LPCSTR in_name, xr_vector<char *> *list, xr_vector<char *> *fl_list, BOOL bFast, BOOL make_pack,
                  LPCSTR copy_path)
{
...
void ProcessLTX(LPCSTR tgt_name, LPCSTR params, BOOL bFast)
{
    ...
    string_path ltx_fn;
    string_path tmp;
    strncpy(tmp, params, ltx_nm - params);
    tmp[ltx_nm - params] = 0;
    _Trim(tmp);
    strcat(tmp, ".ltx");
    strcpy(ltx_fn, tmp);
