...
static lzo_byte *STORE_RUN(lzo_byte *op, const lzo_byte *ii, lzo_uint t, lzo_byte *out)
{
    if (t < 4 && op > out)
        op[-2] |= LZO_BYTE(t);
    else if (t <= 31)
...
LZO_EXTERN(int)
lzo1f_999_compress_callback(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem,
                            lzo_progress_callback_t cb, lzo_uint max_chain);
                            ...
                            lzo_progress_callback_t cb, lzo_uint max_chain)
{
    ...
    while (c->look > 0)
    {
        ...
        if ((m_len < M2_MIN_LEN) || (m_len < M3_MIN_LEN && m_off > M2_MAX_OFFSET))
        {
            m_len = 0;
        }
        else
        {
            ...
            assert(lzo_memcmp(c->ip - c->look, c->ip - c->look - m_off, m_len) == 0);
        ...
        if (m_len > 0 && lazy_match_min_gain >= 0 && c->look > m_len)
        {
            ...
            if (m_len <= M2_MAX_LEN && m_off <= M2_MAX_OFFSET && c->m_off > M2_MAX_OFFSET)
            {
                lazy_match_min_gain += 1;
            }
            else if (c->m_len <= M2_MAX_LEN && c->m_off <= M2_MAX_OFFSET && m_off > M2_MAX_OFFSET)
            {
                if (lazy_match_min_gain > 0)
                    lazy_match_min_gain -= 1;
            }
            else if (m_len == M2_MIN_LEN && c->m_len == M2_MIN_LEN && c->m_off <= 2 * M2_MAX_OFFSET &&
                     m_off > M2_MAX_OFFSET)
            {
                if (lazy_match_min_gain > 0)
                    lazy_match_min_gain -= 1;
...
LZO_PUBLIC(int)
lzo1f_999_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem)
{
    return lzo1f_999_compress_callback(in, in_len, out, out_len, wrkmem, (lzo_progress_callback_t)0, 0);
}
