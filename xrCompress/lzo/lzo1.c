...
LZO_PUBLIC(int)
lzo1_decompress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem)
{
    lzo_byte *op;
    const lzo_byte *ip;
...
static lzo_byte *store_run(lzo_byte *op, const lzo_byte *ii, lzo_uint r_len)
{
    assert(r_len > 0);
...
static int do_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem)
{
    const lzo_byte *ip;
...
#define PS *m_pos++ != *ip++
            ...
            if (PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS)
#elif (MIN_MATCH_LONG - MIN_MATCH == 30) /* MBITS == 5 */
            if (PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS || PS ||
...
#error "MBITS not yet implemented"
#endif
            {
                ...
                *op++ = LZO_BYTE(((m_len - THRESHOLD) << OBITS) | (m_off & OMASK));
...
LZO_PUBLIC(int)
lzo1_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem)
{
    int r = LZO_E_OK;
