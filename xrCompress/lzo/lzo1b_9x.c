...
static lzo_byte *code_match(LZO_COMPRESS_T *c, lzo_byte *op, lzo_uint m_len, lzo_uint m_off)
{
    if (m_len <= M2_MAX_LEN && m_off <= M2_MAX_OFFSET)
    {
        ...
        *op++ = LZO_BYTE(((m_len - (M2_MIN_LEN - 2)) << M2O_BITS) | (m_off & M2O_MASK));
...
LZO_EXTERN(int)
lzo1b_999_compress_callback(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem,
...
LZO_PUBLIC(int)
lzo1b_999_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem)
{
    return lzo1b_999_compress_callback(in, in_len, out, out_len, wrkmem, (lzo_progress_callback_t)0, 0);
}
