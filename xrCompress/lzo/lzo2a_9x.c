...
LZO_EXTERN(int)
lzo2a_999_compress_callback(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem,
    ...
    while (c->look > 0)
    {
        ...
        if (lazy_match_min_gain > 0 && c->look > m_len)
        {
            ...
            if (m_off < 8192 && c->m_off >= 8192)
                ...
                if (m_len >= M1_MIN_LEN && m_len <= M1_MAX_LEN && m_off <= 256)
            {
                if (!(c->m_len >= M1_MIN_LEN && c->m_len <= M1_MAX_LEN && c->m_off <= 256))
...
LZO_PUBLIC(int)
lzo2a_999_compress(const lzo_byte *in, lzo_uint in_len, lzo_byte *out, lzo_uintp out_len, lzo_voidp wrkmem)
{
    return lzo2a_999_compress_callback(in, in_len, out, out_len, wrkmem, (lzo_progress_callback_t)0, 0);
}
