...
static unsigned char Fields_Of_Interest[] = {HOSTNAME_KEY,   HOSTPORT_KEY,     NUMPLAYERS_KEY,
                                             MAXPLAYERS_KEY, MAPNAME_KEY,      GAMETYPE_KEY,
                                             GAMEVER_KEY,    PASSWORD_KEY,     G_USER_PASSWORD_KEY,
#ifdef BATTLEYE
                                             G_BATTLEYE_KEY,
#endif // BATTLEYE
                                             DEDICATED_KEY,  GAMETYPE_NAME_KEY};
...
XRGAMESPY_API ServerBrowser xrGS_ServerBrowserNew(SBBool bLAN_Only, ServerBrowserCallback callback, void *instance)
{
    ...
    return ServerBrowserNew(GAMESPY_GAMENAME, GAMESPY_GAMENAME, SecretKey, 0, GAMESPY_BROWSER_MAX_UPDATES,
...
XRGAMESPY_API SBError xrGS_ServerBrowserUpdate(ServerBrowser sb, SBBool async, SBBool disconnectOnComplete,
...
XRGAMESPY_API void xrGS_ServerBrowserSort(ServerBrowser sb, SBBool ascending, const char *sortkey,
...
XRGAMESPY_API SBError xrGS_ServerBrowserAuxUpdateServer(ServerBrowser sb, SBServer server, SBBool async,
                                                        SBBool fullUpdate)
{
    return ServerBrowserAuxUpdateServer(sb, server, async, fullUpdate);
}

XRGAMESPY_API SBError xrGS_ServerBrowserAuxUpdateIP(ServerBrowser sb, const gsi_char *ip, unsigned short port,
                                                    SBBool viaMaster, SBBool async, SBBool fullUpdate)
{
    return ServerBrowserAuxUpdateIP(sb, ip, port, viaMaster, async, fullUpdate);
}

XRGAMESPY_API const gsi_char *xrGS_SBServerGetPlayerStringValue(SBServer server, int playernum, const gsi_char *key,
...
XRGAMESPY_API double xrGS_SBServerGetPlayerFloatValue(SBServer server, int playernum, const gsi_char *key,
                                                      double fdefault)
{
    return SBServerGetPlayerFloatValue(server, playernum, key, fdefault);
}

XRGAMESPY_API const gsi_char *xrGS_SBServerGetTeamStringValue(SBServer server, int teamnum, const gsi_char *key,
                                                              const gsi_char *sdefault)
{
...
XRGAMESPY_API SBBool xrGS_SBServerGetConnectionInfo(ServerBrowser sb, SBServer server, gsi_u16 PortToConnectTo,
                                                    char *ipstring)
{
