...
GHIEncryptionResult ghttpEncryptorSslEncryptFunc(struct GHIConnection *connection, struct GHIEncryptor *theEncryptor,
                                                 ...
                                                 char *theEncryptedBuffer, int *theEncryptedLength)
{
    ...
    encodeResult = matrixSslEncode(connection->encryptor.mInterface, (unsigned char *)thePlainTextBuffer,
                                   *thePlainTextLength, &encryptedBuf);
...
GHIEncryptionResult ghttpEncryptorSslDecryptFunc(struct GHIConnection *connection, struct GHIEncryptor *theEncryptor,
...
static GHIEncryptionResult ghttpEncryptorProcessSSLHandshake(struct GHIConnection *connection,
                                                             struct GHIEncryptor *encryptor, GHIBuffer *data);
...
static void ghttpEncryptorGenerateEncryptionKeys(gsSSL *sslInterface)
{
    ...
    RC4Init(&sslInterface->sendRC4, (const unsigned char *)sslInterface->clientWriteKey,
...
GHIEncryptionResult ghttpEncryptorSslInitFunc(struct GHIConnection *connection, struct GHIEncryptor *theEncryptor)
{
    ...
    ghiEncryptorWriteNBOLength(helloMsg.header.lengthNBO, sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg),
                               2);
...
//    for tag specifics.
GHIEncryptionResult ghttpEncryptorProcessSSLHandshake(struct GHIConnection *connection, struct GHIEncryptor *encryptor,
                                                      GHIBuffer *data)
{
            ...
            while (data->pos < certListEndPos)
            {
                ...
                if (certCount == 1)
                {
                    ...
                    sslInterface->serverpub.modulus.mLength =
                        (unsigned int)((temp - 1) /
                                       (GS_LARGEINT_BYTE_SIZE / 8)); //-1 to subtract the previous 0x00 byte
                    memcpy(((char *)sslInterface->serverpub.modulus.mData) + (4 - (temp - 1) % 4) % 4,
                    ...
                    sslInterface->serverpub.exponent.mLength =
                        (unsigned int)(((temp - 1) / (GS_LARGEINT_BYTE_SIZE / 8)) + 1); // ceiling of temp/4
                    memcpy(((char *)sslInterface->serverpub.exponent.mData) + (4 - temp % 4) % 4,
...
#pragma message("!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
            ...
            ghiEncryptorWriteNBOLength(clientKeyExchange->header.lengthNBO,
                                       (int)(sslInterface->serverpub.modulus.mLength * (GS_LARGEINT_BYTE_SIZE / 8) + 4),
                                       2);
            clientKeyExchange->handshakeType = GS_SSL_HANDSHAKE_CLIENTKEYEXCHANGE;
            ghiEncryptorWriteNBOLength(clientKeyExchange->lengthNBO,
                                       (int)(sslInterface->serverpub.modulus.mLength * (GS_LARGEINT_BYTE_SIZE / 8)), 3);
            ...
            ghiEncryptorWriteNBOLength((unsigned char *)&connection->sendBuffer.data[connection->sendBuffer.len], 36,
                                       3);
            ...
            MD5Update(&sslInterface->finishHashMD5, (unsigned char *)sslInterface->mastersecret,
            ...
            SHA1Input(&sslInterface->finishHashSHA1, (unsigned char *)sslInterface->mastersecret,
                      GS_SSL_MASTERSECRET_LEN);
