    ...
    /*
        ...
        static void gsiResolveHostnameThread(void * arg)
        {
            ...
            else
            {
                ...
                result = sceNetResolverStartNtoA(resolverID, info->hostname, &info->ip, GSI_RESOLVER_TIMEOUT,
    GSI_RESOLVER_RETRY); if (result < 0) info->ip = GSI_ERROR_RESOLVING_HOSTNAME; sceNetResolverDelete(resolverID);
            }
        }
    #endif
    */
...
#define _LEAP_YEAR_ADJUST 17L
                              ...
                              int hr, int mn, int sc)
    {
        ...
        tmptim =
            ((((tmptim) * 365L + ((long)(yr - 1) >> 2) - (long)_LEAP_YEAR_ADJUST + (long)tmpdays) * 24L + (long)hr) *
                 60L +
             (long)mn) *
                60L +
            (long)sc;
...
#include <net/if_ether.h>
    ...
    static const char *GetMAC(void)
    {
        ...
        int result = sceInetInterfaceControl(&gGSIInsockClientData, &gGSIInsockSocketBuffer, 1, sceInetCC_GetHWaddr,
                                             MAC, sizeof(MAC));
...
#define REG_KEY "Software\\GameSpy\\GameSpy 3D\\Registration"
    ...
    const char *GOAGetUniqueID_Internal(void)
    {
        ...
        if (ret != 0 || strlen(keyval) != 19) // need to generate a new key
        {
            ...
            if (docreate)
            {
                ret = RegCreateKeyExA(HKEY_CURRENT_USER, REG_KEY, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS,
                                      NULL, &thekey, &disp);
            }
