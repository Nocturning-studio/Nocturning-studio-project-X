...
static gsi_bool gsiLargeIntKMult(const gsi_u32 *data1, const gsi_u32 *data2, gsi_u32 length, gsi_u32 *dest,
                                 gsi_u32 *lenout, gsi_u32 maxlen);
static gsi_bool gsiLargeIntMult(const gsi_u32 *data1, gsi_u32 length1, const gsi_u32 *data2, gsi_u32 length2,
                                gsi_u32 *dest, gsi_u32 *lenout, gsi_u32 maxlen);
static gsi_bool gsiLargeIntDiv(const gsi_u32 *src1, gsi_u32 length1, const gsLargeInt_t *divisor, gsLargeInt_t *dest,
                               gsLargeInt_t *remainder);

// Dest may be data1 or data2 to support in-place arithmetic
static gsi_bool gsiLargeIntAdd(const gsi_u32 *data1, gsi_u32 length1, const gsi_u32 *data2, gsi_u32 length2,
                               gsi_u32 *dest, gsi_u32 *lenout, gsi_u32 maxlen);
static gsi_bool gsiLargeIntSub(const gsi_u32 *amount, gsi_u32 length1, const gsi_u32 *from, gsi_u32 length2,
                               gsi_u32 *dest, gsi_u32 *lenout);

// Special division, removes divisor directly from src1, leaving remainder
static gsi_bool gsiLargeIntSubDivide(gsi_u32 *src1, gsi_u32 length, const gsi_u32 *divisor, gsi_u32 dlen,
...
// gsLargeInt_t *dest);
gsi_bool gsiLargeIntMultM(gsLargeInt_t *src1, gsLargeInt_t *src2, const gsLargeInt_t *mod, gsi_u32 modPrime,
                          gsLargeInt_t *dest);
...
{
    gsi_bool result = gsiLargeIntAdd(src1->mData, src1->mLength, src2->mData, src2->mLength, dest->mData,
                                     &dest->mLength, GS_LARGEINT_INT_SIZE);
...
gsi_bool gsiLargeIntSub(const gsi_u32 *src1, gsi_u32 length1, const gsi_u32 *src2, gsi_u32 length2, gsi_u32 *dest,
                        gsi_u32 *lenout)
{
...
{
    gsi_bool result = gsiLargeIntMult(src1->mData, src1->mLength, src2->mData, src2->mLength, dest->mData,
                                      &dest->mLength, GS_LARGEINT_INT_SIZE);
    if (gsi_is_false(result))
        memset(dest, 0, sizeof(gsLargeInt_t)); // overflow
    return result;
...
gsi_bool gsLargeIntDiv(const gsLargeInt_t *src1, const gsLargeInt_t *divisor, gsLargeInt_t *dest,
                       gsLargeInt_t *remainder)
{
...
static gsi_bool gsiLargeIntDiv(const gsi_u32 *src, gsi_u32 len, const gsLargeInt_t *div, gsLargeInt_t *dest,
                               gsLargeInt_t *remainder)
{
    ...
    do
    {
        ...
        else
        {
            ...
            //	_asm {int 3}
            if (gsi_is_false(gsiLargeIntSubDivide(&scopy[readIndex], (gsi_u32)readLength, divisorData, divisorLen,
                                     ...
                                     gsi_u32 highbit, gsi_u32 *quotient)
{
    ...
    while (gsiLargeIntCompare(temp.mData, temp.mLength, src, length) == 1)
    {
        ...
        if (gsi_is_false(gsiLargeIntMult(divisor, dlen, quotientCopy.mData, quotientCopy.mLength, temp.mData,
                                         &temp.mLength, GS_LARGEINT_INT_SIZE)))
        {
            GSLINT_EXITTIMER(GSLintTimerSubDivide);
            return gsi_false; // overflow
        }
    ...
    else
    {
        ...
        if (temp1.mLength > halfLen || temp2.mLength > halfLen)
            gsiLargeIntMult(temp1.mData, temp1.mLength, temp2.mData, temp2.mLength, &temp3.mData[halfLen],
                            &temp3.mLength, GS_LARGEINT_INT_SIZE - halfLen);
        else
        {
        ...
        gsiLargeIntSub(&dest[length], length, &temp3.mData[halfLen], temp3.mLength - halfLen, &temp3.mData[halfLen],
                       &temp3.mLength);
        temp3.mLength += halfLen;
        gsiLargeIntSub(dest, length, &temp3.mData[halfLen], temp3.mLength - halfLen, &temp3.mData[halfLen],
                       &temp3.mLength);
        temp3.mLength += halfLen;
...
gsi_bool gsLargeIntSquareMod(const gsLargeInt_t *lint, const gsLargeInt_t *mod, gsLargeInt_t *dest)
{
    ...
    result =
        gsiLargeIntAdd(otherSums, otherLen, squareSums, squareLen, squareSums, &squareLen, GS_LARGEINT_INT_SIZE * 2);
...
gsi_bool gsiLargeIntMultM(gsLargeInt_t *x, gsLargeInt_t *y, const gsLargeInt_t *m, gsi_u32 modPrime, gsLargeInt_t *dest)
{
    ...
    for (i = 0; (gsi_u32)i < m->mLength; i++)
    {
        ...
        if (gsi_is_false(gsiLargeIntMult(temp.mData, temp.mLength, y->mData, y->mLength, xiy.mData, &xiy.mLength,
                                         GS_LARGEINT_INT_SIZE)))
        {
            // overflow
            dest->mLength = 0;
        ...
        //    Add both to A
        if (gsi_is_false(
                gsiLargeIntAdd(xiy.mData, xiy.mLength, A.mData, A.mLength, A.mData, &A.mLength, GS_LARGEINT_INT_SIZE)))
        {
            // overflow
            dest->mLength = 0;
            dest->mData[0] = 0;
        ...
        if (gsi_is_false(gsiLargeIntAdd(temp.mData, temp.mLength, A.mData, A.mLength, A.mData, &A.mLength,
                                        GS_LARGEINT_INT_SIZE)))
        {
            // overflow
            dest->mLength = 0;
