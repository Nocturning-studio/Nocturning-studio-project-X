...
{
    slist->servers = ArrayNew(sizeof(SBServer), SERVER_GROWBY,
                              NULL); // don't free the server automatically - it goes into the dead list
    slist->deadlist = NULL;
}
...
SBError SBServerListConnectAndQuery(SBServerList *slist, const char *fieldList, const char *serverFilter, int options,
                                    int maxServers)
{
    SBError err;
    int ret;
...
// enable broadcasting where needed
    ...
    for (i = startport; i <= endport; i += 1)
    {
        ...
        if (queryversion == QVERSION_QR2) // send a QR2 echo request
            sendto(slist->slsocket, (char *)qr2_echo_request, qr2requestlen, 0, (struct sockaddr *)&saddr,
                   sizeof(saddr));
...
static int ParseServer(SBServerList *slist, SBServer server, char *buf, int len, int usepopularlist)
{
    ...
    if ((flags & (HAS_FULL_RULES_FLAG | HAS_KEYS_FLAG)) == 0 &&
        ((server->state & STATE_BASICKEYS) | (server->state & STATE_FULLKEYS)))
...
static SBError ProcessLanData(SBServerList *slist)
{
    ...
    {
#if defined(_PS3)
        error = (int)recvfrom(slist->slsocket, indata, sizeof(indata) - 1, 0, (struct sockaddr *)&saddr,
                              (socklen_t *)&saddrlen);
#else
