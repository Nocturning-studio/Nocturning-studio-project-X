...
void TW_Save(ID3D11Texture2D* T, LPCSTR name, LPCSTR prefix, LPCSTR postfix)
{
	string256 fn;
...
*/
ID3D11Resource* CRender::texture_load(LPCSTR fRName, u32& ret_msize, bool bStaging)
{
...
_DDS : {
	...
	if (IMG.MiscFlags & D3D11_RESOURCE_MISC_TEXTURECUBE)
		goto _DDS_CUBE;
	else
		goto _DDS_2D;
...
_DDS_CUBE : {
	...
	R_CHK(D3DX11CreateTextureFromMemory(HW.pDevice11, S->pointer(), S->length(), &LoadInfo, 0, &pTexture2D, 0));
...
_DDS_2D : {
	...
	R_CHK2(D3DX11CreateTextureFromMemory(HW.pDevice11, S->pointer(), S->length(), &LoadInfo, 0, &pTexture2D, 0), fn);
