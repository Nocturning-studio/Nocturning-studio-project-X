...
void CRender::Screenshot		(IRender_interface::ScreenshotMode mode, LPCSTR name)
{
	...
	if (!psWindowMode == 3)//((psDeviceFlags.test(rsFullscreen)) == 0) 
	{
		...
		case IRender_interface::SM_NORMAL:
			{
				...
				if (strstr(Core.Params, "-ss_jpg")) {
					sprintf_s(buf, sizeof(buf), "ssq_%s_%s_(%s).jpg", Core.UserName, timestamp(t_stemp), (g_pGameLevel) ? g_pGameLevel->name().c_str() : "mainmenu");
					ID3DXBuffer* saved = 0;
