...
void	CBlender_luminance::Compile(CBlender_Compile& C)
{
	...
	case 0:				// 256x256	=> 64x64
		C.r_Pass		("null", "postprocess_stage_bloom_luminance_pass_1",false,FALSE,FALSE,	FALSE);
		C.r_Sampler_clf	("s_image",	r2_RT_bloom1);
		C.r_End			();
		break;
	case 1:				// 64x64	=> 8x8
		C.r_Pass		("null", "postprocess_stage_bloom_luminance_pass_2",false,FALSE,FALSE,	FALSE);
		C.r_Sampler_clf	("s_image",	r2_RT_luminance_t64);
		C.r_End			();
		break;
	case 2:				// 8x8		=> 1x1, blending with old result
		C.r_Pass		("null", "postprocess_stage_bloom_luminance_pass_3",false,FALSE,FALSE, FALSE);
