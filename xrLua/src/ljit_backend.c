...
#define jit_next_jmp(J) (jit_assert(GET_OPCODE(*J->nextins) == OP_JMP), (++J->nextpc) + GETARG_sBx(*J->nextins++))
...
static int jit_compile_proto(jit_State *J)
{
    ...
    while (J->nextpc <= lastpc)
    {
        ...
        case OP_MOVE:
            ...
            break;
        case OP_LOADK:
            jit_op_loadk(J, ra, rbx);
            break;
        case OP_LOADBOOL:
            jit_op_loadbool(J, ra, rb, rc);
            break;
        case OP_LOADNIL:
            jit_op_loadnil(J, ra, rb);
            break;

        case OP_GETUPVAL:
            jit_op_getupval(J, ra, rb);
            break;
        case OP_SETUPVAL:
            jit_op_setupval(J, ra, rb);
            break;

        case OP_GETGLOBAL:
            jit_op_getglobal(J, ra, rbx);
            break;
        case OP_SETGLOBAL:
            jit_op_setglobal(J, ra, rbx);
            break;

        case OP_NEWTABLE:
            jit_op_newtable(J, ra, rb, rc);
            break;
        case OP_GETTABLE:
            jit_op_gettable(J, ra, rb, rc);
            break;
        case OP_SETTABLE:
            jit_op_settable(J, ra, rb, rc);
            break;
        case OP_SELF:
            jit_op_self(J, ra, rb, rc);
            break;
        case OP_SETLIST:
            jit_op_setlist(J, ra, rb, rc);
            break;

        case OP_ADD:
            jit_op_arith(J, ra, rb, rc, TM_ADD, 0);
            break;
        case OP_SUB:
            jit_op_arith(J, ra, rb, rc, TM_SUB, 0);
            break;
        case OP_MUL:
            jit_op_arith(J, ra, rb, rc, TM_MUL, 0);
            break;
        case OP_DIV:
            jit_op_arith(J, ra, rb, rc, TM_DIV, 0);
            break;
        case OP_MOD:
            jit_op_arith(J, ra, rb, rc, TM_MOD, 0);
            break;
        case OP_POW:
            jit_op_arith(J, ra, rb, rc, TM_POW, 0);
            break;
        ...
        case OP_CONCAT:
            jit_op_concat(J, ra, rb, rc);
            break;

        case OP_EQ:
            jit_op_eq(J, ra, rb, rc, jit_next_jmp(J));
            break;
        case OP_LT:
            jit_op_arith(J, ra, rb, rc, TM_LT, jit_next_jmp(J));
            break;
        case OP_LE:
            jit_op_arith(J, ra, rb, rc, TM_LE, jit_next_jmp(J));
            break;

        case OP_TEST:
            jit_op_test(J, rc, ra, ra, jit_next_jmp(J));
            break;
        case OP_TESTSET:
            jit_op_test(J, rc, ra, rb, jit_next_jmp(J));
            break;

        case OP_JMP:
            jit_op_jmp(J, J->nextpc + rbx - MAXARG_sBx);
            break;

        case OP_CALL:
            jit_op_call(J, ra, rb - 1, rc - 1);
            break;
        case OP_TAILCALL:
            jit_op_tailcall(J, ra, rb - 1);
            break;
        case OP_RETURN:
            jit_op_return(J, ra, rb - 1);
            break;

        case OP_FORLOOP:
            jit_op_forloop(J, ra, J->nextpc + rbx - MAXARG_sBx);
            break;
        case OP_FORPREP:
            jit_op_forprep(J, ra, J->nextpc + rbx - MAXARG_sBx);
            break;

        case OP_TFORLOOP:
            jit_op_tforloop(J, ra, rc, jit_next_jmp(J));
            break;
        ...
        case OP_CLOSE:
            ...
            break;
        case OP_CLOSURE:
            jit_op_closure(J, ra, rbx);
            break;

        case OP_VARARG:
            jit_op_vararg(J, ra, rb - 1);
            break;
        ...
        if (dasm_checkstep(Dst, DASM_SECTION_CODE))
        {
            J->nextpc--;
            goto asmerr;
        }
...
int luaJIT_backend(lua_State *L)
{
    ...
    {
        J->pt = clvalue(cl)->l.p;
        if (J->pt->sizecode > LUAJIT_LIM_BYTECODE)
        { /* Hard backend limit. */
            status = JIT_S_TOOLARGE;
        }
        else if (J->pt->jit_sizemcode != 0)
...
void luaJIT_freebackend(lua_State *L)
{
    ...
    J->L = L;
    if (J->jsub)
        luaM_freearray(L, J->jsub, JSUB_MAX, void *);
