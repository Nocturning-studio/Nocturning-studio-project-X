...
{
    if (J->mcodeheap)
        HeapDestroy((HANDLE)J->mcodeheap);
}
...
static void *mcode_alloc(jit_State *J, size_t sz)
{
    ...
    for (;;)
    {
        MCodeHead *mh = (MCodeHead *)J->mcodeheap;
        if (mh)
        { /* Got at least one heap so search free list. */
#ifdef MCH_DEBUG
            int slen = 0;
...
static void mcode_free_(jit_State *J, void *ptr, size_t sz)
{
    ...
    if (!(tr & MCH_FIRST))
    {
        ...
        if (!(*pptrailer & MCH_USED))
        { /* Prev free? */
            if (!(tr & MCH_LAST) && !(*trailer & MCH_USED))
            { /* Next free? */
...
#define mcode_free(L, J, p, sz) mcode_free_(J, (p), MCH_ROUNDSIZE((sz) + sizeof(size_t)))
...
{
    if (J->jsubmcode)
        luaM_freemem(J->L, J->jsubmcode, J->sizejsubmcode);
}
...
int luaJIT_link(jit_State *J, void **mcodep, size_t *szp)
{
    ...
    if ((J->dasmstatus = dasm_link(Dst, &sz)))
        return JIT_S_DASM_ERROR;

    /* Check for hardcoded limit on mcode size. */
    if (sz > LUAJIT_LIM_MCODE)
        return JIT_S_TOOLARGE;
