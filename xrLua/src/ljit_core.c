...
int luaJIT_currentpc(lua_State *L, Proto *pt, const Instruction *savedpc)
{
    ...
    if (pc >= 0 && pc <= pt->sizecode)
    { /* Bytecode pointer? */
        return (int)pc - 1;
    }
    else
    { /* Else translate JIT mcode address to PC. */
...
static int jit_compile(lua_State *L, StkId func, int nargs, int force)
{
    ...
    if ((J->flags & JIT_F_INIT_FAILED) || (!force && !(J->flags & JIT_F_ON)) || !J->frontwrap)
    {
    ...
    cl->l.jit_gate = (status == JIT_S_OK) ? (lua_CFunction)pt->jit_mcode : G(L)->jit_gateJL;
...
int luaJIT_setmode(lua_State *L, int idx, int mode)
{
    ...
    case LUAJIT_MODE_DEBUG: { /* Set debug mode. */
        ...
        case 0:
            ...
            break;
        case 1:
            dbg = JIT_F_DEBUG_CALL;
            break;
        case 2:
        default:
            dbg = JIT_F_DEBUG_CALL | JIT_F_DEBUG_INS;
            break;
        }
    ...
    case LUAJIT_MODE_ALLSUBFUNC: { /* Set mode for subfunctions. */
        StkId func;
        lua_lock(L);
        func = idx == 0 ? (L->ci - 1)->func : (idx > 0 ? L->base + (idx - 1) : L->top + idx);
        if (isLfunction(func))
        {
            Closure *cl = clvalue(func);
            Proto *pt = cl->l.p;
