...
static void pusherror(lua_State *L)
{
    ...
    char buffer[128];
    if (FormatMessage(FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM, NULL, error, 0, buffer,
                      sizeof(buffer), NULL))
...
{
    luaL_error(L, "error loading module " LUA_QS " (%s)", lua_tostring(L, 1), lua_tostring(L, -1));
}
...
static int ll_require(lua_State *L)
{
    ...
    for (i = 1;; i++)
    {
        ...
        if (lua_isnil(L, -1))
            lua_pop(L, 1); /* did not found module */
        else
