...
/* jit.* functions. */
static const luaL_Reg jitlib[] = {{"on", j_on},
                                  {"off", j_off},
                                  {"debug", j_debug},
                                  {"compile", j_compile},
...
static int j_frontend(lua_State *L)
{
    ...
    for (pos = 1;; pos += 2)
    {
        ...
        { /* Pending failure? */
            ...
            lua_pop(L, 1);
            if ((prio & 1) == 0)
                continue; /* Skip handlers with even priority. */
        }
    ...
    if (pc >= 1 && pc <= pt->sizecode)
    {
        ...
        case iABC: {
            ...
            if (getBMode(op) == OpArgN)
                ...
                lua_pushinteger(L, ISK(b) ? (-1 - INDEXK(b)) : b);
            if (getCMode(op) == OpArgN)
                lua_pushnil(L);
            else
                lua_pushinteger(L, ISK(c) ? (-1 - INDEXK(c)) : c);
...
static int ju_const(lua_State *L)
{
    ...
    if (idx < 0)
        idx = -idx; /* Handle both positive and negative indices. */
...
static int ju_mcode(lua_State *L)
{
    Proto *pt = check_LCL(L)->l.p;
    if (pt->jit_sizemcode == 0)
    { /* Not compiled (yet): return nil, status. */
        lua_pushnil(L);
...
/* jit.util.* functions. */
static const luaL_Reg jitutillib[] = {{"stats", ju_stats},         {"bytecode", ju_bytecode},
                                      {"const", ju_const},         {"upvalue", ju_upvalue},
                                      {"closure", ju_closure},     {"closurenup", ju_closurenup},
                                      {"mcode", ju_mcode},         {"mcodeaddr", ju_mcodeaddr},
                                      {"jsubmcode", ju_jsubmcode}, {NULL, NULL}};

/* Make hint name to hint number map. */
static void makehints(lua_State *L, const char *const *t, int tmax, const char *name)
{
    int i;
...
static const char *const status_list[] = {"OK",       "NONE",           "OFF",       "ENGINE_OFF", "DELAYED",
                                          "TOOLARGE", "COMPILER_ERROR", "DASM_ERROR"};
...
LUALIB_API int LUA_CC luaopen_jit(lua_State *L)
{
    ...
    if (luaJIT_setmode(L, 0, LUAJIT_MODE_ENGINE | LUAJIT_MODE_ON) <= 0)
        ...
        return luaL_error(L, "JIT engine init failed (%08x)", G(L)->jit_state->dasmstatus);
