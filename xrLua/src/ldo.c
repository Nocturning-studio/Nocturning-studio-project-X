...
static StkId adjust_varargs(lua_State *L, Proto *p, int actual)
{
    ...
    {                                 /* compat. with old-style vararg? */
        ...
        setnvalue(luaH_setstr(L, htab, luaS_newliteral(L, "n")), cast(lua_Number, nvar));
    }
#endif
...
#define inc_ci(L) ((L->ci == L->end_ci) ? luaD_growCI(L) : (condhardstacktests(luaD_reallocCI(L, L->size_ci)), ++L->ci))
