...
void dasm_free(Dst_DECL)
{
    ...
    if (D->labels)
        dasm_m_free(Dst, D->labels, D->lsize);
...
void dasm_put(Dst_DECL, int start, ...)
{
    ...
    if (pos >= sec->epos)
    {
        dasm_m_grow(Dst, (void **)&sec->buf, &sec->bsize, sec->bsize + 2 * DASM_MAXSECPOS * sizeof(int));
    ...
    while (1)
    {
        ...
        else
        {
            ...
            putlabel:
                ...
                n = *pl;
                while (n > 0)
                { /* Collapse rel chain and replace with label pos. */
        ...
        while (b != endb)
        {
            ...
            while (1)
            {
                ...
                case DASM_IMM_L:
                    p++;
                case DASM_IMM_P:
                    n = (ptrdiff_t)base + *DASM_POS2PTR(D, n);
                    goto wd;
                case DASM_LABEL_L:
                    p++;
                case DASM_LABEL_P:
...
void dasm_getlabels(Dst_DECL, void *base, void **pcaddr)
{
    ...
    for (pc = 0; pc < D->maxpc; pc++)
    {
        ...
        if (pos >= 0)
            ...
            pcaddr[pc] = (void *)((unsigned char *)base + *DASM_POS2PTR(D, -pos));
    }
}
