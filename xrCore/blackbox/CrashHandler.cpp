...
BOOL InternalSymGetLineFromAddr(IN HANDLE hProcess, IN DWORD dwAddr, OUT PDWORD pdwDisplacement,
...
LPCTSTR __stdcall GetFaultReason(EXCEPTION_POINTERS *pExPtrs)
{
    ...
    __try
    {
        ...
        DWORD dwDisp;
        if (TRUE == SymGetSymFromAddr((HANDLE)GetCurrentProcessId(), (DWORD)pExPtrs->ExceptionRecord->ExceptionAddress,
                                      &dwDisp, pSym))
        {
            ...
            else
            {
                if (dwDisp > 0)
                {
                    iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s()+%04d byte(s)" ), pSym->Name, dwDisp);
                }
                else
                {
        ...
        if (TRUE == InternalSymGetLineFromAddr((HANDLE)GetCurrentProcessId(),
                                               (DWORD)pExPtrs->ExceptionRecord->ExceptionAddress, &dwDisp, &g_stLine))
        {
            iCurr += wsprintf(g_szBuff + iCurr, _T ( ", " ));
...
BOOL __stdcall CH_ReadProcessMemory(HANDLE, LPCVOID lpBaseAddress, LPVOID lpBuffer, DWORD nSize,
...
#define CH_MACHINE IMAGE_FILE_MACHINE_I386
        ...
        BOOL bSWRet = StackWalk(CH_MACHINE, (HANDLE)GetCurrentProcessId(), GetCurrentThread(), &g_stFrame,
                                pExPtrs->ContextRecord, (PREAD_PROCESS_MEMORY_ROUTINE)CH_ReadProcessMemory,
        ...
        dwModBase = SymGetModuleBase((HANDLE)GetCurrentProcessId(), g_stFrame.AddrPC.Offset);
        if (0 == dwModBase)
        {
            szRet = NULL;
            return (szRet);
        ...
        if (GSTSO_SYMBOL == (dwOpts & GSTSO_SYMBOL))
        {
            ...
            if (TRUE == SymGetSymFromAddr((HANDLE)GetCurrentProcessId(), g_stFrame.AddrPC.Offset, &dwDisp, pSym))
            {
                ...
                else
                {
                    if (dwDisp > 0)
                    {
                        iCurr += wsprintf(g_szBuff + iCurr, _T( "%s()"), pSym->Name, dwDisp);
                    }
                    else
                    {
                        iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s" ), pSym->Name);
        ...
        if (GSTSO_SRCLINE == (dwOpts & GSTSO_SRCLINE))
        {
            ...
            if (TRUE ==
                InternalSymGetLineFromAddr((HANDLE)GetCurrentProcessId(), g_stFrame.AddrPC.Offset, &dwDisp, &g_stLine))
            {
                iCurr += wsprintf(g_szBuff + iCurr, _T ( ", " ));
                ...
                else
                {
                    ...
                    {
                        iCurr +=
                            wsprintf(g_szBuff + iCurr, _T("%s, %d"), g_stLine.FileName, g_stLine.LineNumber, dwDisp);
                    }
                    else
                    {
...
void InitSymEng(void)
{
    if (FALSE == g_bSymEngInit)
    {
        ...
        VERIFY(BSUSymInitialize((DWORD)hPID, hPID, g_application_path, TRUE));
        g_bSymEngInit = TRUE;
    }
}
