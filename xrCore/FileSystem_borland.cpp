...
BOOL EFS_Utils::CheckLocking(LPCSTR fname, bool bOnlySelf, bool bMsg) //, shared_str* owner)
{
	...
	{
		HANDLE handle =
			CreateFile(fn, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
		CloseHandle(handle);
...
void EFS_Utils::WriteAccessLog(LPCSTR fn, LPCSTR start_msg)
{
	/*
		...
		sprintf			(buf, "%s:   '%s' from computer: '%s' by user: '%s' at %s
	   %s",start_msg,fn,Core.CompName,Core.UserName,_strdate(dt_buf),_strtime(tm_buf)); xr_string		m_AccessLog;
