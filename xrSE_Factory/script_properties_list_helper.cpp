...
ChooseValue *CScriptPropertiesListHelper::CreateChoose(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u32 mode, LPCSTR path, LPCSTR fill_param,
                                                       u32 sub_item_cnt)
{
...
FloatValue *CScriptPropertiesListHelper::CreateFloat(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, float mn, float mx, float inc, int decim)
{
    return (PHelper().CreateFloat(*items, key, wrap_value<float>(object, name), mn, mx, inc, decim));
}

FloatValue *CScriptPropertiesListHelper::CreateFloat(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, float mn, float mx, float inc)
{
    return (PHelper().CreateFloat(*items, key, wrap_value<float>(object, name), mn, mx, inc));
}

FloatValue *CScriptPropertiesListHelper::CreateFloat(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, float mn, float mx)
{
    return (PHelper().CreateFloat(*items, key, wrap_value<float>(object, name), mn, mx));
}

FloatValue *CScriptPropertiesListHelper::CreateFloat(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, float mn)
{
...
VectorValue *CScriptPropertiesListHelper::CreateVector(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, float mn, float mx, float inc, int decim)
{
...
Flag8Value *CScriptPropertiesListHelper::CreateFlag8(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, u8 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{
    return (PHelper().CreateFlag8(*items, key, wrap_value<Flags8>(object, name), mask, c0, c1, flags));
}

Flag8Value *CScriptPropertiesListHelper::CreateFlag8(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, u8 mask, LPCSTR c0, LPCSTR c1)
{
    return (PHelper().CreateFlag8(*items, key, wrap_value<Flags8>(object, name), mask, c0, c1));
}

Flag8Value *CScriptPropertiesListHelper::CreateFlag8(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, u8 mask, LPCSTR c0)
{
...
Flag16Value *CScriptPropertiesListHelper::CreateFlag16(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u16 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{
    return (PHelper().CreateFlag16(*items, key, wrap_value<Flags16>(object, name), mask, c0, c1, flags));
}

Flag16Value *CScriptPropertiesListHelper::CreateFlag16(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u16 mask, LPCSTR c0, LPCSTR c1)
{
    return (PHelper().CreateFlag16(*items, key, wrap_value<Flags16>(object, name), mask, c0, c1));
}

Flag16Value *CScriptPropertiesListHelper::CreateFlag16(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u16 mask, LPCSTR c0)
{
    return (PHelper().CreateFlag16(*items, key, wrap_value<Flags16>(object, name), mask, c0));
}

Flag16Value *CScriptPropertiesListHelper::CreateFlag16(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u16 mask)
{
    return (PHelper().CreateFlag16(*items, key, wrap_value<Flags16>(object, name), mask));
}

Flag32Value *CScriptPropertiesListHelper::CreateFlag32(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u32 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{
    return (PHelper().CreateFlag32(*items, key, wrap_value<Flags32>(object, name), mask, c0, c1, flags));
}

Flag32Value *CScriptPropertiesListHelper::CreateFlag32(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u32 mask, LPCSTR c0, LPCSTR c1)
{
    return (PHelper().CreateFlag32(*items, key, wrap_value<Flags32>(object, name), mask, c0, c1));
}

Flag32Value *CScriptPropertiesListHelper::CreateFlag32(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u32 mask, LPCSTR c0)
{
    return (PHelper().CreateFlag32(*items, key, wrap_value<Flags32>(object, name), mask, c0));
}

Flag32Value *CScriptPropertiesListHelper::CreateFlag32(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, u32 mask)
{
    return (PHelper().CreateFlag32(*items, key, wrap_value<Flags32>(object, name), mask));
}

Token8Value *CScriptPropertiesListHelper::CreateToken8(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, CScriptTokenList *token)
{
    return (PHelper().CreateToken8(*items, key, wrap_value<u8>(object, name), &*token->tokens().begin()));
}

Token16Value *CScriptPropertiesListHelper::CreateToken16(PropItemVec *items, LPCSTR key, luabind::object object,
                                                         LPCSTR name, CScriptTokenList *token)
{
    return (PHelper().CreateToken16(*items, key, wrap_value<u16>(object, name), &*token->tokens().begin()));
}

Token32Value *CScriptPropertiesListHelper::CreateToken32(PropItemVec *items, LPCSTR key, luabind::object object,
                                                         LPCSTR name, CScriptTokenList *token)
{
...
RListValue *CScriptPropertiesListHelper::CreateRList(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, CScriptRTokenList *tokens)
{
    return (PHelper().CreateRList(*items, key, wrap_value<shared_str>(object, name), &*tokens->tokens().begin(),
                                  tokens->size()));
}
...
ColorValue *CScriptPropertiesListHelper::CreateFColor(PropItemVec *items, LPCSTR key, luabind::object object,
                                                      LPCSTR name)
{
    return (PHelper().CreateFColor(*items, key, wrap_value<Fcolor>(object, name)));
}

VectorValue *CScriptPropertiesListHelper::CreateVColor(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name)
{
    return (PHelper().CreateVColor(*items, key, wrap_value<Fvector>(object, name)));
}

RTextValue *CScriptPropertiesListHelper::CreateRText(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name)
{
...
FloatValue *CScriptPropertiesListHelper::CreateAngle(PropItemVec *items, LPCSTR key, luabind::object object,
                                                     LPCSTR name, float mn, float mx, float inc, int decim)
{
...
VectorValue *CScriptPropertiesListHelper::CreateAngle3(PropItemVec *items, LPCSTR key, luabind::object object,
                                                       LPCSTR name, float mn, float mx, float inc, int decim)
{
