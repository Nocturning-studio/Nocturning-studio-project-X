...
void CSE_Abstract::FillProps(LPCSTR pref, PropItemVec &items)
{
    ...
    //	PHelper().CreateFloat		(items,PrepareKey(pref,*s_name,"Spawn\\probability"),
    //&m_spawn_probability,	0.f,			1.f); 	PHelper().CreateFlag32
    //(items,PrepareKey(pref,*s_name,"Spawn\\spawn on surge only"),		&m_spawn_flags,			flSpawnOnSurgeOnly);
    //	PHelper().CreateFlag32		(items,PrepareKey(pref,*s_name,"Spawn\\spawn if destroyed only"),	&m_spawn_flags,
    //flSpawnIfDestroyedOnly); 	PHelper().CreateFlag32		(items,PrepareKey(pref,*s_name,"Spawn\\spawn infinite
    //count"),		&m_spawn_flags,			flSpawnInfiniteCount); 	PHelper().CreateFlag32
    //(items,PrepareKey(pref,*s_name,"Spawn\\auto destroy on spawn"),		&m_spawn_flags,
    //flSpawnDestroyOnSpawn); . #	endif // DEBUG . #endif // XRGAME_EXPORTS
}
