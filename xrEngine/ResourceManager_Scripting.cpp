...
class adopt_sampler
{
  ...
  public:
	adopt_sampler(CBlender_Compile* _C, LPCSTR name) : C(_C)
	{
	...
	module(LSVM)
		...
		class_<adopt_sampler>("_sampler")
		 ...
		 class_<adopt_blend>("blend").enum_(
			 "blend")[value("zero", int(D3DBLEND_ZERO)), value("one", int(D3DBLEND_ONE)),
		 ...
		 class_<adopt_stencil_op>("stencil_op")
			 .enum_(
				 "stencil_op")[value("keep", int(D3DSTENCILOP_KEEP)), value("zero", int(D3DSTENCILOP_ZERO)),
...
Shader* CResourceManager::_lua_Create(LPCSTR d_shader, LPCSTR s_textures)
{
	...
	if (Script::bfIsObjectPresent(LSVM, s_shader, "normal_hq", LUA_TFUNCTION))
	{
		...
		C.bDetail = Device.Resources->m_textures_description.GetDetailTexture(
