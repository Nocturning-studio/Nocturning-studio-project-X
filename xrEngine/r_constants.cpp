...
BOOL R_constant_table::parseConstants(ID3D11ShaderReflectionConstantBuffer* pTable, u32 destination)
{
	// VERIFY(_desc);
	VERIFY(pTable);
...
BOOL R_constant_table::parseResources(ID3D11ShaderReflection* pReflection, int ResNum, u32 destination)
{
	...
	if (ShaderDesc.ConstantBuffers)
	{
		...
		for (u16 iBuf = 0; iBuf < ShaderDesc.ConstantBuffers; ++iBuf)
		{
			...
			if (pTable)
			{
				...
				ref_cbuffer tempBuffer = Device.Resources->_CreateConstantBuffer(pTable);
