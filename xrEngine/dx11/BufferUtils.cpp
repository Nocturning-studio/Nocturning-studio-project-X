...
{
HRESULT CreateVertexBuffer(ID3D11Buffer** ppBuffer, const void* pData, UINT DataSize, bool bImmutable)
{
	...
	HRESULT res = HW.pDevice11->CreateBuffer(&desc, &subData, ppBuffer);
	return res;
}

HRESULT CreateIndexBuffer(ID3D11Buffer** ppBuffer, const void* pData, UINT DataSize, bool bImmutable)
{
	D3D11_BUFFER_DESC desc;
	desc.ByteWidth = DataSize;
	desc.Usage = D3D11_USAGE_DEFAULT;
...
HRESULT CreateConstantBuffer(ID3D11Buffer** ppBuffer, UINT DataSize)
{
	...
	desc.MiscFlags = 0;
	HRESULT res = HW.pDevice11->CreateBuffer( &desc, 0, ppBuffer);
	return res;
}
