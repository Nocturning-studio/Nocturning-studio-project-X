...
void WeaponUsageStatistic::Write_WeaponEfficiency(FILE *pFile)
{
    ...
    for (i = 0; i < aWeapons.size(); i++)
    {
        ...
        for (u32 j = 0; j < aPlayersStatistic.size(); j++)
        {
            ...
            //			sprintf(tmpStr1, "%s%d\t", tmpStr1, (NumHitsPlayer > 0) ? int(.5f +
            //float(NumKillsPlayer)/NumHitsPlayer*100) : 0);
            sprintf(tmpStr1, "%s%.2f\t", tmpStr1, (NumHitsPlayer > 0) ? float(NumKillsPlayer) / NumHitsPlayer : 0);
        };
        //		fprintf(pFile, "%s\t%d\t%s\n", tmpStr0, (NumHitsTotal>0) ?int(.5f +
        //float(NumKillsTotal)/NumHitsTotal*100):0, tmpStr1);
        ConvOut(pFile, "%s\t%.2f\t%s\n", tmpStr0, (NumHitsTotal > 0) ? float(NumKillsTotal) / NumHitsTotal : 0,
                tmpStr1);
    }
    //-----------------------------------------------------
    fprintf(pFile, "\n");
...
void WeaponUsageStatistic::Write_WeaponAccuracy(FILE *pFile)
{
    ...
    for (i = 0; i < aWeapons.size(); i++)
    {
        ...
        for (u32 j = 0; j < aPlayersStatistic.size(); j++)
        {
            ...
            //			sprintf(tmpStr1, "%s%d\t", tmpStr1, (NumBulletsFired > 0) ?
            //int(.5f+float(NumHitsScored)/NumBulletsFired*100) : 0);
            sprintf(tmpStr1, "%s%.2f\t", tmpStr1, (NumBulletsFired > 0) ? (float(NumHitsScored) / NumBulletsFired) : 0);
        };
        //		fprintf(pFile, "%s\t%d\t%s\n", tmpStr0, (NumBulletsFiredTotal > 0) ?
        //int(.5f+float(NumHitsScoredTotal)/NumBulletsFiredTotal*100) : 0, tmpStr1);
        ConvOut(pFile, "%s\t%.2f\t%s\n", tmpStr0,
                (NumBulletsFiredTotal > 0) ? float(NumHitsScoredTotal) / NumBulletsFiredTotal : 0, tmpStr1);
    }
    //-----------------------------------------------------
    fprintf(pFile, "\n");
}
...
void WeaponUsageStatistic::Write_EffectiveRange(FILE *pFile)
{
    ...
    for (i = 0; i < aWeapons.size(); i++)
    {
        ...
        {
            fprintf(pFile, "%d\t",
                    HitsOnRange[k]); // (HitsTotal>0) ? int(.5f + float(HitsOnRange[k])/HitsTotal*100) : 0);
        }
        fprintf(pFile, "\n");
    }
    //-----------------------------------------------------
...
void WeaponUsageStatistic::Write_AverageMoney(FILE *pFile)
{
    ...
    for (int t = 0; t < 3; t++)
    {
        ...
        {
        case 0:
            fprintf(pFile, "DM Team\t");
            break;
        case 1:
            fprintf(pFile, "Green Team\t");
            break;
        case 2:
            fprintf(pFile, "Blue Team\t");
            break;
        };

        ConvOut(pFile, "%.2f\t",
                (m_dwTotalNumRespawns[t] > 0) ? (float(m_dwTotalPlayersMoneyRound[t]) / m_dwTotalNumRespawns[t]) : 0);

        for (u32 i = 0; i < aPlayersStatistic.size(); i++)
        {
            Player_Statistic &PS = aPlayersStatistic[i];
            ConvOut(pFile, "%.2f\t",
                    (PS.m_dwNumRespawned[t] > 0) ? (float(PS.m_dwTotalMoneyRound[t]) / PS.m_dwNumRespawned[t]) : 0);
        }
    ...
    for (int t = 0; t < 3; t++)
    {
        ...
        for (u32 i = 0; i < aPlayersStatistic.size(); i++)
        {
            ...
            for (u32 j = 0; j < PS.aWeaponStats.size(); j++)
            {
                ...
                {
                    if (WS.m_Basket[t][k] > 0 && MaxBasketPos < k)
                        MaxBasketPos = k;
                }
            }
        };
...
void WeaponUsageStatistic::Write_AverageLifeSpan(FILE *pFile)
{
    ...
    for (int t = 0; t < 3; t++)
    {
        ...
        ConvOut(pFile, "%.2f\t",
                (m_dwTotalNumRespawns[t] > 0) ? (float(m_dwTotalPlayersAliveTime[t]) / m_dwTotalNumRespawns[t] / 1000)
                                              : 0);
        ...
        for (u32 i = 0; i < aPlayersStatistic.size(); i++)
        {
            Player_Statistic &PS = aPlayersStatistic[i];
            ConvOut(pFile, "%.2f\t",
                    (PS.m_dwNumRespawned[t] > 0) ? (float(PS.m_dwTotalAliveTime[t]) / PS.m_dwNumRespawned[t] / 1000)
                                                 : 0);
        }
