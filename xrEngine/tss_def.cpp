...
void SimulatorStates::UpdateDesc(D3D11_RASTERIZER_DESC& desc) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
		const State& S = States[it];
...
void SimulatorStates::UpdateDesc(D3D11_DEPTH_STENCIL_DESC& desc) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
		const State& S = States[it];
...
void SimulatorStates::UpdateDesc(D3D11_BLEND_DESC& desc) const
{
	for (u32 it = 0; it < States.size(); it++)
	{
		const State& S = States[it];
		if (S.type == 0)
		{
			...
			case D3DRS_SRCBLEND:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].SrcBlend = (D3D11_BLEND)S.v2;
				break;

			case D3DRS_DESTBLEND:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].DestBlend = (D3D11_BLEND)S.v2;
				break;
			...
			case D3DRS_BLENDOP:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].BlendOp = (D3D11_BLEND_OP)S.v2;
				break;

			case D3DRS_SRCBLENDALPHA:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].SrcBlendAlpha = (D3D11_BLEND)S.v2;
				break;

			case D3DRS_DESTBLENDALPHA:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].DestBlendAlpha = (D3D11_BLEND)S.v2;
				break;

			case D3DRS_BLENDOPALPHA:
				for (int i = 0; i < 8; ++i)
					desc.RenderTarget[i].BlendOpAlpha = (D3D11_BLEND_OP)S.v2;
				break;
...
void SimulatorStates::UpdateDesc(D3D11_SAMPLER_DESC descArray[D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT],
								 bool SamplerUsed[D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT], int iBaseSamplerIndex) const
{
	...
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		if (S.type == 2)
		{
			...
			if ((iSamplerIndex >= D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT) || iSamplerIndex < 0)
				continue;

			SamplerUsed[iSamplerIndex] = true;
			D3D11_SAMPLER_DESC& desc = descArray[iSamplerIndex];
			...
			case D3DSAMP_MAGFILTER: /* D3DTEXTUREFILTER filter to use for magnification */
				...
				case D3DTEXF_NONE:
				case D3DTEXF_POINT:
					desc.Filter = (D3D11_FILTER)(desc.Filter & (~MagfilterLinear));
					break;
				case D3DTEXF_LINEAR:
					desc.Filter = (D3D11_FILTER)(desc.Filter | MagfilterLinear);
			...
			case D3DSAMP_MINFILTER: /* D3DTEXTUREFILTER filter to use for minification */
				...
				case D3DTEXF_POINT:
					...
					desc.Filter = (D3D11_FILTER)(desc.Filter & (~MinfilterLinear));
					break;
				case D3DTEXF_LINEAR:
					desc.Filter = (D3D11_FILTER)(desc.Filter | MinfilterLinear);
					// desc.Filter |= MinfilterLinear;
			...
			case D3DSAMP_MIPFILTER: /* D3DTEXTUREFILTER filter to use between mipmaps during minification */
				...
				case D3DTEXF_NONE:
				case D3DTEXF_POINT:
					desc.Filter = (D3D11_FILTER)(desc.Filter & (~MipfilterLinear));
			...
			case XRDX10SAMP_ANISOTROPICFILTER:
				if (S.v3)
					desc.Filter = (D3D11_FILTER)(desc.Filter | FilterAnisotropic);
			...
			case XRDX10SAMP_COMPARISONFILTER:
				if (S.v3)
					desc.Filter = (D3D11_FILTER)(desc.Filter | FilterComparison);
			...
			case D3DSAMP_ADDRESSU: /* D3DTEXTUREADDRESS for U coordinate */
				desc.AddressU = (D3D11_TEXTURE_ADDRESS_MODE)S.v3;
				break;

			case D3DSAMP_ADDRESSV: /* D3DTEXTUREADDRESS for V coordinate */
				desc.AddressV = (D3D11_TEXTURE_ADDRESS_MODE)S.v3;
				break;

			case D3DSAMP_ADDRESSW: /* D3DTEXTUREADDRESS for W coordinate */
				desc.AddressW = (D3D11_TEXTURE_ADDRESS_MODE)S.v3;
				break;
			...
			case XRDX10SAMP_COMPARISONFUNC:
				desc.ComparisonFunc = (D3D11_COMPARISON_FUNC)S.v3;
				break;
	...
	{
		D3D11_SAMPLER_DESC& desc = descArray[i];
		if (desc.Filter & FilterAnisotropic)
		{
			desc.Filter = (D3D11_FILTER)(desc.Filter | AllfilterLinear);
